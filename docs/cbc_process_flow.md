...
| **1 - File setup** | Create job workbook | Excel (password-protected; credential not stored in repo) | New job workbook | `intake-coordinator` |
| **2 - Spec scoping** | Identify Div 08 and Div 10 scope | PDF specs | Confirmed scope, rating and HW notes | `spec-scope-analyst` |
| **3 - Take-offs** | Counts, sizes, handing, rating, keying | Edge viewer / Vu360 | Quantities and opening attributes | `takeoff-engineer` |
| **3b - FRP** | Wall-panel measurement | Vu360 + calculator | FRP geometry + product attrs | `frp-specialist` |
| **3c - Div 10** | Partitions / accessories / dryers | Specs + schedules | Specialty counts | `div10-specialist` |
| **4 - Pricing and build** | Populate and price the quote | Excel + P21 + vendor sheets / RFQ | Priced draft quote | `product-matcher`, `pricing-engineer` (HTML is worker-rendered; `quote-builder` is interactive/headless only) |
...
| 3c | `extracted/div10_takeoff.json` |
