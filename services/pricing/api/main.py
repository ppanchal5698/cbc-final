...
log = logs.configure("cbc.pricing.api")
...
async def lifespan(app: FastAPI):
    ...
    otel.configure("cbc.pricing.api")
    ...
    log.info("pricing api ready; storage at %s", settings.storage_root)
    sweep_task = None
    try:
        yield
    finally:
...
for router in (reference_data.router,):
