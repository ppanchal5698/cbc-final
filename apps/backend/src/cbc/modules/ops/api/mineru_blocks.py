...
def looks_like_middle(payload: Any) -> bool:
    """True when a MinerU payload carries page content (not a status stub).

    Task status envelopes often look like ``{status, result_url, task_id}`` with
    no ``middle_json`` / ``pdf_info``. Treating those as a successful parse left
    ``documentPages`` empty while ``parse.state`` stayed ``parsed``.
    """
    if isinstance(payload, list):
        return any(isinstance(p, dict) for p in payload)
    if not isinstance(payload, dict):
        return False
    if payload.get("middle_json") is not None or payload.get("pdf_info") is not None:
        return True
    if _pages_from_middle(payload):
        return True
    # GET /tasks/{id}/result → {results: {filename: {middle_json, md_content}}}
    results = payload.get("results")
    if isinstance(results, dict):
        for entry in results.values():
            if isinstance(entry, dict) and (
                entry.get("middle_json") is not None
                or entry.get("pdf_info") is not None
                or _pages_from_middle(entry)
            ):
                return True
    return False


def unwrap_mineru_payload(payload: Any) -> Any:
    """Normalize MinerU status/result envelopes to a middle-json-shaped dict.

    Status stubs are returned unchanged (caller should fetch ``result_url``).
    Result URLs wrap content as ``{results: {<filename>: {middle_json}}}``.
    """
    if not isinstance(payload, dict):
        return payload
    if payload.get("middle_json") is not None or payload.get("pdf_info") is not None:
...
def normalise_window(
    ...
    *,
    ...
    data = unwrap_mineru_payload(load_middle(middle))
