...
def test_admin_delete_purges_mongo_and_disk(client) -> None:
    ...
    if FIXTURE_PDF.exists():
        ...
        # Uploading the bid set is what enqueues a job; creating a project does
        # not. The fixture PDF is not committed, so seed the job the upload would
        # have queued - otherwise the purge below has no job to prove it removes.
        seed = mongo_client(serverSelectionTimeoutMS=5000)
        try:
            seed[TEST_DB]["jobs"].insert_one(
                {"type": "extract_bid_set", "projectId": project_id, "status": "queued", "attempts": 0}
            )
        finally:
            seed.close()
