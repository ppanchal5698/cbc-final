...
5. The retired kernel stays retired: there is no `cbc.db`, `services/`, `schemas/`,
   `pageindex/`, `persistence/`, `domain/`, `core/`, `validation/`, `http/`, `api/` or `worker/` beside the modules, and nothing imports any of them.
...
def test_the_retired_kernel_stays_retired() -> None:
    """`cbc.db`, `cbc.services`, `cbc.schemas`, `cbc.pageindex`, `cbc.persistence`, `cbc.domain`, `cbc.core` and `cbc.validation` held what the modules and `shared` own now; none comes back."""
    assert not any((SRC / name).exists() for name in ("db.py", "services", "schemas", "pageindex", "persistence", "domain", "core", "validation", "http", "api", "worker"))
    importers = [
        ...
        for line, dotted in _imports(path)
        if re.match(r"cbc\.(db|services|schemas|pageindex|persistence|domain|core|validation|http|api|worker)(\.|$)", dotted)
    ]
